...
uint32_t _VISCA_write_packet_data(VISCAInterface_t *iface, VISCACamera_t *camera, VISCAPacket_t *packet)
{
	...
	for (i = 0; i < packet->length; i++) {
		fprintf(stderr, "packet->bytes[%d] = %x\n", i, packet->bytes[i]);
	}
#endif
...
uint32_t _VISCA_get_packet(VISCAInterface_t *iface)
{
	...
	while (1) {
		FD_ZERO(&rfds);
		FD_SET(iface->port_fd, &rfds);

		// 设置超时时间
		tv.tv_sec = UART_TIMEOUT_MS / 1000;
		tv.tv_usec = (UART_TIMEOUT_MS % 1000) * 1000;

		// 等待串口可读
		ret = select(iface->port_fd + 1, &rfds, NULL, NULL, &tv);
		if (ret < 0) {
#if VISCA_LIB_DEBUG
			fprintf(stderr, "_VISCA_get_packet: select error\n");
#endif
			return VISCA_FAILURE;
		} else if (ret == 0) {
			// 超时
		...
		if (FD_ISSET(iface->port_fd, &rfds)) {
			// 读取一个字节
			bytes_read = read(iface->port_fd, &iface->ibuf[pos], 1);
			if (bytes_read < 0) {
#if VISCA_LIB_DEBUG
				fprintf(stderr, "_VISCA_get_packet: read error\n");
#endif
				return VISCA_FAILURE;
			...
			pos++;
			if (pos >= (int)sizeof(iface->ibuf)) {
#if VISCA_LIB_DEBUG
				fprintf(stderr, "_VISCA_get_packet: buffer overflow\n");
#endif
				return VISCA_FAILURE;
			}
		}
...
uint32_t VISCA_open_serial(VISCAInterface_t *iface, const char *device_name)
{
	...
	if (fd == -1) {
		...
		/* control flags */
		cfsetispeed(&iface->options, B115200); /* 115200 Bds   */
